...
def test_resolve_source_field_ece4(tmp_path):
    ...
    # WOA23 for NO3
    no3_meta = resolve_source_field("NO3", pack="ece4", raw_dir=raw)
    ...
    # Panaïotis DOC
    doc_meta = resolve_source_field("DOC", pack="ece4", raw_dir=raw)
    ...
    # Tagliabue Fer
    fe_meta = resolve_source_field("Fer", pack="ece4", raw_dir=raw)
...
def test_resolve_source_field_official_sette(tmp_path):
    ...
    no3_meta = resolve_source_field("NO3", pack="official_sette", raw_dir=raw)
    assert no3_meta["product"] == "sette_nomask"
    assert no3_meta["src_var"] == "NO3"
    assert "data_NO3_nomask.nc" in no3_meta["src_file"]

    talk_meta = resolve_source_field("TALK", pack="official_sette", raw_dir=raw)
    assert talk_meta["product"] == "sette_nomask"
    assert talk_meta["src_var"] == "TALK"
    assert talk_meta["std_var"] == "Alkalini"
    assert "data_ALK_nomask.nc" in talk_meta["src_file"]

    doc_meta = resolve_source_field("DOC", pack="official_sette", raw_dir=raw)
...
def test_export_env_scripts():
    src_env = export_source_env("DOC", pack="ece4")
    assert "export VAR=\"DOC\"" in src_env
    assert "export HANDLER=\"doc\"" in src_env

    # Test preset backward compatibility alias
    src_env_legacy = export_source_env("DOC", preset="ece4")
    assert src_env == src_env_legacy
