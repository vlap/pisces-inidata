...
def pad_abyssal_depth(in_file, out_file, target_bottom_depth=6000.0):
    ...
    with nc.Dataset(in_file, 'r') as src, nc.Dataset(out_file, 'w', format='NETCDF4') as dst:
        ...
        for dname, dim in src.dimensions.items():
            if pad_needed and dname == z_dim:
                dst.createDimension(dname, len(dim) + 1 if not dim.isunlimited() else None)
            else:
        ...
        for vname, var in src.variables.items():
            ...
            if pad_needed and vname == z_var:
                new_depths = np.append(depth_vals, np.float32(target_bottom_depth))
                out_var[:] = new_depths
            elif pad_needed and z_dim in out_dims:
                z_axis = out_dims.index(z_dim)
                data = var[:]
                last_slice = np.take(data, indices=[-1], axis=z_axis)
                padded_data = np.concatenate([data, last_slice], axis=z_axis)
                out_var[:] = padded_data
            else:
                out_var[:] = var[:]

    print(f"Abyssal padding completed: {out_file} (bottom depth: {target_bottom_depth}m)")
