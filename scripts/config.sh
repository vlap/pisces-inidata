...
# ------------------------------------------------------------------------------
PRESET="${PRESET:-${INIDATA_PRESET:-${INIDATA_PACK:-ece4}}}"
export PRESET INIDATA_PRESET

CONFIG_FILE="${PISCES_CONFIG:-${CONFIG_FILE:-}}"
if [ -z "${CONFIG_FILE}" ]; then
    if [ -f "${REPO_DIR}/config/packs/${PRESET}.yaml" ]; then
        CONFIG_FILE="${REPO_DIR}/config/packs/${PRESET}.yaml"
    elif [ -f "${REPO_DIR}/sources.yaml" ]; then
